...
class CLIP4ClipPreTrainedModel(PreTrainedModel, nn.Module):
    ...
    def from_pretrained(cls, cross_model_name, state_dict=None, cache_dir=None, type_vocab_size=2, *inputs, **kwargs):
        print("111111 classmethod  from_pretrained")
        task_config = None
        ...
        print('[start creat instance]...................')
        model = cls(cross_config, clip_state_dict, *inputs, **kwargs) # -----------
        print('[finish model instance]...................')
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        transformer_layers = len(set(k.split(".")[2] for k in clip_state_dict if k.startswith(f"transformer.resblocks"))) # Transformer 模型中的层数
    ...
    def dot_product_logits(self, sequence_output, visual_output, text_mask, video_mask):
        sequence_output = self.get_text_sep_feat(sequence_output, text_mask) 
        if self.training:
        ...
        visual_output = self._mean_pooling_for_similarity_video(visual_output, video_mask)   
        visual_output = visual_output / visual_output.norm(dim=-1, keepdim=True)
    ...
    def get_video_text_similarity_logits(self, text_feat, video_feat, title_feat, text_mask, video_mask, title_mask, loose_type=False):
        ...
        visual_output = self.agg_video_feat(video_feat, video_mask, self.sim_header)
    ...
    def get_text_title_similarity_logits(self, text_output, title_output, text_mask, title_mask):
        ...
        text_output = self.get_text_sep_feat(text_output, text_mask).squeeze(1) 
        title_output = self.get_text_sep_feat(title_output, title_mask).squeeze(1)
        if self.training:
        ...
        retrieve_logits = torch.matmul(text_output, title_output.t())  
        if self.training:
            logit_scale = self.clip.logit_scale.exp()
            retrieve_logits = logit_scale * retrieve_logits
            return retrieve_logits
